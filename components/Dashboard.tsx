...
import { ThumbsUp, ThumbsDown, Flag, MessageSquare, X, Navigation, Clock, Route as RouteIcon, Shield, ChevronUp, Zap, Bell, BellOff, ThermometerSun, Users, AlertTriangle, Target, Link as LinkIcon, Map, Mountain, Moon, Car, Train, Bike, Layers } from 'lucide-react';
...
export const Dashboard: React.FC<Props> = ({ user, onLogout, onViewHistory, onViewProfile, onMessages, onZoneSafety, onParentalTracking }) => {
  ...
  const { teriaState, setTeriaState, host: teriaHost, setHost: setTeriaHost, port: teriaPort, setPort: setTeriaPort, lastMessage, testConnection } = useTeria((lat, lng, alt) => {
     simulationService.injectTeriaPosition(lat, lng, alt, 0.5); // 0.5m accuracy for TERIA
  });
    ...
    <div className="flex h-[100vh] bg-slate-950 text-slate-100 font-sans relative flex-col md:flex-row overflow-y-auto overflow-x-hidden">
      ...
      `}>
       ...
       <div 
         ...
         className={`flex items-center p-3 cursor-pointer text-slate-400 hover:text-white transition-colors select-none ${isSidebarCollapsed ? 'justify-center border-b-0' : 'justify-between'}`}
         title={isSidebarCollapsed ? 'TERIA Config' : undefined}
       >
         <div className="flex items-center gap-2">
           <Zap className="w-4 h-4 shrink-0" />
           {!isSidebarCollapsed && <span className="text-xs font-bold uppercase tracking-wider">TERIA Config</span>}
         </div>
