...
import { ThumbsUp, ThumbsDown, Flag, MessageSquare, X, Navigation, Clock, Route as RouteIcon, Shield } from 'lucide-react';
...
export const Dashboard: React.FC<Props> = ({ user, onLogout, onViewHistory, onViewProfile }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans relative">
      ...
      {/* Sidebar */}
      <div className={`
        fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 border-r border-slate-800/50 flex flex-col overflow-hidden transition-transform duration-300 ease-in-out shadow-2xl
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden w-full">
        {/* Top Bar */}
        <div className="h-16 border-b border-slate-800 flex items-center px-4 md:px-6 justify-between bg-slate-900/50 backdrop-blur shrink-0 gap-4">
        ...
        <div className="flex-1 p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 lg:grid-rows-2 gap-4 md:gap-6 overflow-y-auto lg:overflow-hidden">
          ...
          <div className="col-span-1 lg:col-span-2 lg:row-span-2 relative min-h-[500px] lg:min-h-0 h-[60vh] lg:h-auto glass-panel rounded-2xl overflow-hidden shadow-2xl">
             <MapVisualization state={state} dynamicZoom={dynamicZoom} layerVisibility={layerVisibility} nearbyUsers={nearbyUsers} activeAlerts={activeAlerts} denmMessages={denmMessages} />
             
             {/* Route Info Overlay */}
             {state?.route && (
               <div className="absolute top-4 left-4 z-[1000] bg-slate-900/90 backdrop-blur-md border border-slate-700 p-4 rounded-xl shadow-2xl max-w-xs w-full">
                 <div className="flex items-center justify-between mb-3">
                   ...
                   <button 
                     onClick={() => simulationService.clearRoute()}
