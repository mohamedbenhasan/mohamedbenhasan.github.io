...
export const Dashboard: React.FC<Props> = ({ user, onLogout, onViewHistory, onViewProfile, onMessages, onZoneSafety, onParentalTracking }) => {
  ...
  return (
    <div className="flex h-[100vh] bg-slate-950 text-slate-100 font-sans relative flex-col md:flex-row overflow-y-auto overflow-x-hidden">
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/50 z-[90000] md:hidden backdrop-blur-sm transition-opacity"
      ...
      {/* Sidebar */}
      <div className={`
        fixed inset-y-0 left-0 z-[90010] bg-slate-950 border-r border-slate-800/50 flex flex-col overflow-hidden transition-all duration-300 ease-in-out shadow-2xl
        ...
        <div className={`flex-1 overflow-y-auto relative ${isRightPanelCollapsed ? 'p-0 md:p-4 flex flex-col' : 'p-2 md:p-4 grid grid-cols-1 lg:grid-cols-3 lg:grid-rows-2 gap-4 md:gap-6 lg:h-full'}`}>
          ...
          <div className={`transition-all duration-300 glass-panel overflow-hidden shadow-2xl ${
            ...
            <MapVisualization 
             ...
             <div className="absolute bottom-6 left-6 z-[1000] flex flex-col items-start gap-2 pointer-events-none">
                 {voiceAssistantState.statusText && (
                     <div className="bg-black/80 backdrop-blur text-white text-xs px-3 py-2 rounded-lg pointer-events-none shadow-lg border border-slate-700 max-w-[200px] text-left">
                         {voiceAssistantState.statusText}
                     </div>
                 )}
                 <button 
                    ...
                    className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all pointer-events-auto ${voiceAssistantState.isListening ? 'bg-red-500 hover:bg-red-600 animate-pulse text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
                 >
