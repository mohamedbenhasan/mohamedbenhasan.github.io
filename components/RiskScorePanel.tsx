...
export const RiskScorePanel: React.FC<RiskScorePanelProps> = ({ vru, config, onConfigChange, isOpen, onToggle, onOptimize, isOptimizing }) => {
  ...
  if (!isOpen) {
    return (
      <button 
        onClick={onToggle}
        className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-[100000] bg-slate-900 border border-slate-700 text-white px-4 md:px-5 h-12 md:h-14 rounded-full shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2"
      >
        <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
        <span className="font-bold text-sm md:text-base">RISK MODEL</span>
      </button>
    );
  }

  return (
    <div className="fixed bottom-6 right-6 z-[100000] w-96 bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
