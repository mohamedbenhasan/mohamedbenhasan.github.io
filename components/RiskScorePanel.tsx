...
export const RiskScorePanel: React.FC<RiskScorePanelProps> = ({ vru, config, onConfigChange, isOpen, onToggle, onOptimize, isOptimizing }) => {
  ...
  if (!isOpen) {
    return (
      <button 
        onClick={onToggle}
        className="fixed bottom-6 right-6 z-[100000] bg-slate-900 border border-slate-700 text-white p-4 rounded-full shadow-xl hover:bg-slate-800 transition-all flex items-center gap-2"
      >
