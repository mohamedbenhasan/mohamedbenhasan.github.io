...
import { AlertTriangle, Construction, Ban, Zap } from 'lucide-react';
...
const MapClickHandler = ({ onDestinationChange, waitingForIncidentClick, onIncidentClick }: { onDestinationChange?: (dest: { lat: number; lng: number; source: 'click' }) => void, waitingForIncidentClick?: boolean, onIncidentClick?: (latlng: {lat: number, lng: number}) => void }) => {
  useMapEvents({
    click(e) {
      if (waitingForIncidentClick && onIncidentClick) {
        onIncidentClick(e.latlng);
      } else {
        simulationService.setDestination(e.latlng.lat, e.latlng.lng, `Lat: ${e.latlng.lat.toFixed(4)}, Lng: ${e.latlng.lng.toFixed(4)}`);
        if (onDestinationChange) {
          onDestinationChange({ lat: e.latlng.lat, lng: e.latlng.lng, source: 'click' });
        }
      }
    },
  });
...
export const MapVisualization: React.FC<Props> = ({ state, dynamicZoom, layerVisibility, gridOpacity = 0.4, nearbyUsers = [], activeAlerts = [], activeSosMarkers = [], denmMessages = [], onDestinationChange, searchedLocation, reportingLocation: propsReportingLocation, setReportingLocation: propsSetReportingLocation, waitingForIncidentClick: propsWaitingForIncidentClick, setWaitingForIncidentClick: propsSetWaitingForIncidentClick, showLiveOnly = true, pmrAnalysis }) => {
  ...
  const displayVrus = showLiveOnly ? state.vrus.filter(v => v.isUserControlled) : state.vrus;
  ...
  return (
    <div className="relative w-full h-full bg-slate-900 overflow-hidden rounded-xl border border-slate-700 shadow-2xl z-0">
      ...
      >
        ...
        <MapController state={displayState} dynamicZoom={dynamicZoom} />
        <MapClickHandler 
          ...
          <>
            ...
            <Marker 
              ...
              icon={L.divIcon({
                ...
                html: `<div class="flex items-center justify-center w-6 h-6 rounded-full bg-red-500 border-2 border-white shadow-lg text-white">
        ...
        {pmrAnalysis && pmrAnalysis.segments.flatMap((s: any) => s.obstacles).map((obs: any, i: number) => {
          ...
          return (
            <Marker key={`obs-${i}`} position={[obs.location.lat, obs.location.lng]} icon={obsIcon}>
              <Popup>
                <div className="text-slate-800">
                  <strong>Obstacle PMR</strong><br/>
                  <span className="text-xs text-slate-500">Type: {obs.type}</span><br/>
                  {obs.description}
                </div>
              </Popup>
            </Marker>
                  ...
                  {vru.isUserControlled && (
                    <div className="mt-2 pt-2 border-t border-slate-200 text-xs">
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`px-1.5 py-0.5 rounded text-[10px] ${vru.locationSource === 'INDOOR_ANCHOR' ? 'bg-purple-100 text-purple-700 font-bold' : vru.locationSource === 'DEAD_RECKONING' ? 'bg-orange-100 text-orange-700' : 'bg-slate-100'}`}>
                          {vru.locationSource || 'GPS'}
                        </span>
                      </div>
                      {vru.locationFloor !== undefined && (
                        <div><span className="font-semibold">Floor:</span> {vru.locationFloor}</div>
                      )}
                      {vru.locationApartment !== undefined && (
                        <div><span className="font-semibold">Apt:</span> {vru.locationApartment}</div>
                      )}
                    </div>
                  )}
                </div>
