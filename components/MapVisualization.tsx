...
const MapClickHandler = ({ waitingForIncidentClick, onIncidentClick, onMapClick }: { waitingForIncidentClick?: boolean, onIncidentClick?: (latlng: {lat: number, lng: number}) => void, onMapClick?: (latlng: {lat: number, lng: number}) => void }) => {
  useMapEvents({
    click(e) {
      // Prevent handling if the click originated from inside a Leaflet popup or control
...
export const MapVisualization: React.FC<Props> = ({ state, dynamicZoom, setDynamicZoom, layerVisibility, setLayerVisibility, gridOpacity = 0.4, nearbyUsers = [], activeAlerts = [], activeSosMarkers = [], denmMessages = [], onDestinationChange, searchedLocation, destinationLabel, reportingLocation: propsReportingLocation, setReportingLocation: propsSetReportingLocation, waitingForIncidentClick: propsWaitingForIncidentClick, setWaitingForIncidentClick: propsSetWaitingForIncidentClick, pmrAnalysis }) => {
  ...
  const infrastructureLayer = useMemo(() => {
    if (!layerVisibility.infrastructure) return null;
    return infrastructureService.getWays().map(way => {
      let color = '#3b82f6';
      let weight = 3;
      let dashArray = '';
      
      if (way.type === 'footway' || way.type === 'pedestrian' || way.type === 'path') {
        color = '#22c55e'; // green for pedestrian
        weight = 4;
        dashArray = '5, 5'; // dashed for small paths
      } else if (way.type === 'cycleway') {
        color = '#eab308'; // yellow for cycle
        weight = 4;
      } else if (way.type === 'residential' || way.type === 'living_street') {
        color = '#94a3b8'; // grey for small streets
        weight = 6;
      } else if (way.type === 'primary' || way.type === 'secondary' || way.type === 'tertiary') {
        color = '#475569'; // dark grey for main streets
        weight = 8;
      } else if (way.type === 'crossing') {
        color = '#ffffff'; // white crossing
        weight = 8;
        dashArray = '5, 10';
      }
      ...
      return (
        <Polyline 
          ...
          pathOptions={{ color, weight, opacity: 0.6, dashArray, lineCap: 'round', lineJoin: 'round' }}
        >
          <Popup className="custom-popup" minWidth={200}>
            <div className="flex flex-col gap-2 p-1">
              ...
              {way.tags && Object.keys(way.tags).length > 0 && (
                <div className="space-y-1">
                  {way.tags.name && (
                    <div className="text-sm font-semibold text-slate-700 bg-slate-50 px-2 py-1 rounded">
                      {way.tags.name}
                    </div>
                  )}
                  {way.tags.maxspeed && (
                    <div className="flex items-center gap-2 text-xs text-slate-600 bg-slate-50 px-2 py-1 rounded">
                      <span className="font-bold flex items-center justify-center bg-red-100 text-red-600 border-2 border-red-500 rounded-full w-6 h-6 text-[10px]">
                        {way.tags.maxspeed}
                      </span>
                      Max Speed
                    </div>
                  ...
                  {way.tags.surface && (
                    <div className="text-xs text-slate-600 bg-slate-50 px-2 py-1 rounded">
                      <span className="font-semibold text-slate-500">Surface:</span>{' '}
                      {way.tags.surface}
                    </div>
                  )}
                  {way.tags.highway && (
                    <div className="text-xs text-slate-600 bg-slate-50 px-2 py-1 rounded">
                      <span className="font-semibold text-slate-500">Highway:</span>{' '}
                      {way.tags.highway}
                    </div>
                  )}
                </div>
  ...
  }, [layerVisibility.infrastructure, typeof infrastructureService.getWays()]);

  const pmrLayer = useMemo(() => {
    if (!pmrAnalysis) return null;
    return (
      <React.Fragment>
        {/* PMR Path Segments */}
        {pmrAnalysis.segments.map((segment: any, i: number) => {
          let color = '#3b82f6'; // blue (accessible)
          if (segment.accessibilityScore < 0.4) color = '#ef4444'; // red (not accessible)
          else if (segment.accessibilityScore < 0.7) color = '#f59e0b'; // yellow (needs assistance)
        ...
        {/* PMR Obstacles */}
        {pmrAnalysis.segments.flatMap((s: any) => s.obstacles).map((obs: any, i: number) => {
              ...
              <Popup className="custom-popup border-none" minWidth={200}>
                <div className="flex flex-col gap-1.5 p-1 font-sans text-slate-800">
                  ...
                  <div className="text-xs text-slate-600">
                    <span className="font-semibold text-slate-900">Type:</span> {obs.type}
                  </div>
                  <div className="text-xs text-slate-600">
                    <span className="font-semibold text-slate-900">Sévérité:</span> {obs.severity}
                  </div>
                  {onDestinationChange && (
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        onDestinationChange({ lat: obs.location.lat, lng: obs.location.lng, label: `Obstacle PMR`, source: 'click' });
                      }}
  ...
  }, [pmrAnalysis, onDestinationChange]);
    ...
    useMapEvents({
      dragstart: () => {
        if (isHeadingUp) {
          ...
          toast.info("Mode auto-centrage désactivé.");
        }
        if (dynamicZoom && setDynamicZoom) {
          setDynamicZoom(false);
          toast.info("Zoom automatique désactivé.");
        }
      },
      zoomstart: () => {
        if (isHeadingUp) {
  ...
  return (
    <div className="relative w-full h-full bg-slate-900 overflow-hidden rounded-xl border border-slate-700 shadow-2xl z-0">
      ...
      >
        ...
        {selectedPoi && (
          <Marker 
            key={`poi-${selectedPoi.lat}-${selectedPoi.lng}`}
            ...
            icon={L.divIcon({
              className: 'bg-transparent',
              html: `<div class="w-4 h-4 bg-blue-600 rounded-full border-2 border-white shadow-lg animate-pulse" style="margin-top:-8px; margin-left:-8px;"></div>`,
              iconAnchor: [0, 0]
            })}
          >
            <Popup onClose={() => setSelectedPoi(null)} className="custom-popup" autoPan={false} closeButton={false} autoClose={false} closeOnClick={false}>
              <div 
                 className="flex flex-col gap-3 min-w-[240px] p-2 font-sans bg-white text-slate-800 rounded-lg cursor-default"
                  ...
                  <>
                    <div>
                      <strong className="text-sm font-bold text-slate-900 block leading-tight">
                        {selectedPoi.data?.name || selectedPoi.data?.display_name?.split(',')[0] || "Position Sélectionnée"}
                      </strong>
                    ...
                    <div className="flex gap-2">
                       <button 
                         className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium text-xs py-2 px-2 rounded-md transition-colors shadow-sm"
                         onClick={(e) => {
                       ...
                       <button 
                         className="flex-3 bg-blue-600 hover:bg-blue-700 text-white font-medium text-xs py-2 px-3 rounded-md flex items-center justify-center gap-1.5 transition-colors shadow-sm"
                         onClick={(e) => {
                           e.stopPropagation(); e.preventDefault();
