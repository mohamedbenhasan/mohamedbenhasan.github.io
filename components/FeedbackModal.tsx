...
export const FeedbackModal: React.FC<Props> = ({ userId, vruType, onClose, isExitMode = false }) => {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100000] flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-4 bg-slate-950/50 border-t border-slate-800 flex justify-end gap-3">
           ...
           >
             {isExitMode ? 'Ignorer (Déconnexion)' : 'Annuler'}
           </button>
           <button 
           ...
           >
             ...
             {isExitMode ? `Envoyer & ${rating > 0 ? 'Quitter' : 'Se déconnecter'}` : 'Soumettre'}
           </button>
        </div>
      </div>
    </div>
