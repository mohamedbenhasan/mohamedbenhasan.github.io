...
import { advancedGeolocationService, LocationData } from './AdvancedGeolocationService';
...
import { LiveLocation } from './LiveInteractionService';

class SimulationService {
  public updateExternalUsers(liveUsers: LiveLocation[]) {
    // Keep our own controlled user
    const user = this.state.vrus.find(v => v.isUserControlled);
    
    const externalVRUs = liveUsers.map(lu => {
       const existing = this.state.vrus.find(v => v.id === lu.userId);
       
       let velocity = { x: 0, y: 0 };
       let heading = existing?.heading || 0;
       
       // Simple velocity estimation
       if (existing && existing.position.lat !== lu.lat && existing.position.lng !== lu.lng) {
          const dx = lu.lng - existing.position.lng;
          const dy = lu.lat - existing.position.lat;
          // Normalized roughly. Realistically we'd need timestamps to do dx/dt.
          // Using a small fixed magnitude for visualization of "moving" status
          const mag = Math.sqrt(dx*dx + dy*dy);
          if (mag > 0.000001) {
            velocity = { x: (dx/mag) * 2, y: (dy/mag) * 2 };
       ...
       return {
         id: lu.userId,
         type: (lu.type as VRUType) || VRUType.PEDESTRIAN,
         position: { lat: lu.lat, lng: lu.lng },
         geolocation: { current: { lat: lu.lat, lng: lu.lng } },
         velocity: velocity,
         heading: heading,
         sensors: [],
         riskLevel: existing?.riskLevel || RiskLevel.SAFE,
         lastDENMBroadcast: existing?.lastDENMBroadcast,
    ...
    const newVrus = [];
    if (user) newVrus.push(user);
    newVrus.push(...externalVRUs);

    this.state.vrus = newVrus;
    this.notify();
  }
  private state: SimulationState;
  private intervalId: number | null = null;
  private subscribers: ((state: SimulationState) => void)[] = [];
  private isGpsMode: boolean = true;
  ...
  private lastRiskFieldUpdate: number = 0;
  ...
  constructor() {
    ...
    this.enableGpsMode(); // Always enable GPS by default globally upon instantiation
  }
  ...
  public enableGpsMode() {
    ...
    advancedGeolocationService.startWatching(
      (data: LocationData) => {
        this.state.lastGpsError = undefined;
        this.updateUserPositionFromGps(data);
      },
      (error) => {
  ...
  public disableGpsMode() {
    ...
    this.state.lastGpsError = undefined;
    advancedGeolocationService.stopWatching();
    this.notify();
  }

  private isTeriaActive = false;

  public setTeriaActive(active: boolean) {
    this.isTeriaActive = active;
  }

  public injectTeriaPosition(lat: number, lng: number, alt?: number, accuracy: number = 0.5) {
    if (!this.isTeriaActive) return;
    this.updateUserPositionFromGps({
      coords: { lat, lng },
      accuracy,
      source: 'TERIA',
      altitude: alt,
  ...
  private updateUserPositionFromGps(data: LocationData) {
    // If TERIA is active, discard normal GPS updates unless the update IS from TERIA
    if (this.isTeriaActive && data.source !== 'TERIA') {
      return;
    }
    
    const user = this.state.vrus.find(v => v.isUserControlled);
    if (user) {
      user.position = data.coords;
      user.geolocation = { current: data.coords };
      user.localizationError = data.accuracy;
      user.locationSource = data.source;
      user.locationFloor = data.floor;
      user.locationApartment = data.apartment;
      user.locationIsDualBand = data.isDualBand;
      user.locationCorrectionType = data.correctionType;
      user.locationRtkFixed = data.rtkFixed;
      
      // Update GPS sensor reading if it exists
      const gpsSensor = user.sensors.find(s => s.type === 'GPS');
      if (gpsSensor) {
        gpsSensor.reading = data.coords;
        gpsSensor.accuracy = data.accuracy;
      }
      
      // Update Live Tracking Session
      liveTrackingService.updateLocation(data.coords.lat, data.coords.lng);
      
      // Update simulation center to follow user
      this.currentCenter = data.coords;
  ...
  private tick() {
    ...
    const nextVRUs = vrus.map(vru => {
      ...
      const newPos = fusionResult.fusedPos;
      
      // Perform Map Matching
      const prevMatchedWayId = vru.matchedWayId ?? null;
      let matchedPosition = undefined;
      let matchedWayId = undefined;
      let mapMatchingConfidence = undefined;
      
      const matchResult = mapMatchingService.match(
         newPos, 
         vru.heading, 
         vru.position, 
         newPos, 
      ...
      if (matchResult && matchResult.confidence >= 0.6) {
         matchedPosition = matchResult.matchedPosition;
         matchedWayId = matchResult.matchedWayId;
         mapMatchingConfidence = matchResult.confidence;
      }
      ...
      const effectivePosition = matchedPosition || newPos;

      return {
        ...vru,
        position: effectivePosition,
        gpsPosition: newPos, // Track raw position for fallback / debug
        matchedPosition,
        matchedWayId,
        mapMatchingConfidence,
        geolocation: { current: effectivePosition },
        sensors: nextSensors,
    ...
    if (currentRoute && user) {
      ...
      if (distToDest < 10) {
        ...
        import('./AudioGuidanceService').then(({ audioGuidanceService }) => {
          audioGuidanceService.resetProgress();
        });
      } else {
        // Trim route to start from closest point to user
        let minDistanceToRoute = Infinity;
        let closestIndex = 0;
        for (let i = 0; i < currentRoute.coordinates.length; i++) {
          const dist = getDistance(user.position, currentRoute.coordinates[i]);
        ...
        // Handle Audio Guidance
        if (currentRoute.steps && currentRoute.steps.length > 0) {
           import('./AudioGuidanceService').then(({ audioGuidanceService }) => {
             // Find next step based on proximity and progress
             // The AudioGuidanceService tracks currentStepIndex. 
             const currentIndex = audioGuidanceService.getCurrentStepIndex();
             if (currentIndex < currentRoute!.steps!.length) {
               const nextStep = currentRoute!.steps![currentIndex];
               const stepDist = getDistance(user.position, nextStep.location);
               audioGuidanceService.updatePosition(stepDist, nextStep, currentIndex);
               
               // If we are very close to it, advance to next step for future checks
               if (stepDist < 10) {
                  audioGuidanceService.advanceStep();
               }
             }
           });
        }

        // Only trim if we are reasonably close to the route and not at the very beginning
        if (minDistanceToRoute < 20 && closestIndex > 0) {
           currentRoute = {
             ...currentRoute,
             coordinates: [user.position, ...currentRoute.coordinates.slice(closestIndex)]
           };
        }
    ...
    // Throttle calculation to avoid UI freeze, compute every 2 seconds
    let updatedRiskField = riskFieldService.updateRiskField(finalVRUs) || undefined;
    if (now - this.lastRiskFieldUpdate > 2000) {
      this.lastRiskFieldUpdate = now;
      updatedRiskField = riskFieldService.updateRiskField(finalVRUs, this.currentViewport, this.currentZoom) || undefined;
    }
    ...
    if (this.destination && user) {
      ...
      if (currentRoute && currentRoute.coordinates.length > 0) {
        // Check distance to closest point on route
        let minDistanceToRoute = Infinity;
        // Check every Nth point to save computation, or all points if short
        const step = Math.max(1, Math.floor(currentRoute.coordinates.length / 50));
        for (let i = 0; i < currentRoute.coordinates.length; i += step) {
          const dist = getDistance(user.position, currentRoute.coordinates[i]);
          if (dist < minDistanceToRoute) {
            minDistanceToRoute = dist;
          }
        ...
        // If user is more than 20 meters away from the route, they are off-route
        if (minDistanceToRoute > 20) {
          isOffRoute = true;
        }
      }

      // Recalculate if off-route (with a small debounce of 2 seconds to avoid spam) OR periodically every 15 seconds
      if ((isOffRoute && (now - this.lastRouteCalculationTime > 2000)) || (!isOffRoute && (now - this.lastRouteCalculationTime > 15000))) {
        this.lastRouteCalculationTime = now;
        
        if (isOffRoute) {
            import('./AudioGuidanceService').then(({ audioGuidanceService }) => {
                audioGuidanceService.speak("Re-calcul de l'itinéraire en cours.");
            });
        }
  ...
  private routePreference: 'safest' | 'fastest' = 'safest';

  public setRoutePreference(pref: 'safest' | 'fastest') {
     this.routePreference = pref;
     this.updateRoute().catch(console.error);
  }

  private async updateRoute() {
    const user = this.state.vrus.find(v => v.isUserControlled);
    if (!user || (!this.destination && !this.state.route)) return;
    
    // Determine target from existing route or current destination
    let target = this.destination;
    let targetName = this.state.route ? (this.state.route as any).destinationName : undefined;
    
    if (!target && this.state.route && this.state.route.steps.length > 0) {
        const lastStep = this.state.route.steps[this.state.route.steps.length - 1];
        target = lastStep.location;
    }
    
    if (!target) return;
    
    await this.setDestination(target.lat, target.lng, targetName);
  }
  
  public async setDestination(lat: number, lng: number, name?: string) {
    const user = this.state.vrus.find(v => v.isUserControlled);
    if (!user) return null;
    ...
    const preferredRoute = (this.routePreference === 'fastest' && fastest) ? fastest : (safest || fastest);

    if (preferredRoute) {
      this.state.route = preferredRoute;
      if (this.state.route && name) {
          (this.state.route as any).destinationName = name;
      }
      this.state.alternativeRoutes = [];
      
      // Check if safest route avoided incidents compared to fastest
      if (safest && fastest) {
        const activeIncidents = incidentService.getActiveIncidents();
        let fastestIncidents = 0;
        let safestIncidents = 0;
        
        activeIncidents.forEach(incident => {
          const inFastest = fastest.coordinates.some(c => getDistance(c, incident.location) < 50);
          const inSafest = safest.coordinates.some(c => getDistance(c, incident.location) < 50);
          if (inFastest) fastestIncidents++;
          if (inSafest) safestIncidents++;
        });

        if (fastestIncidents > safestIncidents) {
          const avoided = fastestIncidents - safestIncidents;
          if (typeof window !== 'undefined' && (window as any).toast) {
            (window as any).toast.success(`Route ajustée pour éviter ${avoided} incident${avoided > 1 ? 's' : ''}`);
          }
        }
      }
  ...
  public async recalculateRoute() {
    if (this.destination) {
      await this.setDestination(this.destination.lat, this.destination.lng);
    }
  }
