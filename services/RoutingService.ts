import { Coordinates, Route, RiskFieldGrid, GridCell, IncidentSeverity, IncidentType, VRUType } from '../types';
...
export class RoutingService {
  ...
  private currentVruType: VRUType = VRUType.VEHICLE;

  public setRiskWeight(weight: number) {
    this.riskWeight = weight;
  }

  public setVruType(type: VRUType) {
    this.currentVruType = type;
  }

  private getOsrmProfile(): string {
    switch (this.currentVruType) {
      case VRUType.PEDESTRIAN:
      case VRUType.WHEELCHAIR:
        return 'foot';
      case VRUType.CYCLIST:
      case VRUType.SCOOTER:
        return 'bike';
      case VRUType.MOTORCYCLE:
      case VRUType.VEHICLE:
      default:
        return 'driving';
    }
  }
  ...
  private async calculateFastestRoute(start: Coordinates, end: Coordinates): Promise<Route | null> {
    try {
      ...
      const url = `${OSRM_BASE_URL}/${profile}/${start.lng},${start.lat};${end.lng},${end.lat}?overview=full&geometries=geojson&alternatives=true&steps=true`;
      const response = await fetch(url);
      if (!response.ok) throw new Error(`OSRM API Error: ${response.statusText}`);

      const data = await response.json();
      if (data.code !== 'Ok' || !data.routes || data.routes.length === 0) throw new Error('No routes found');
      ...
      // Parse steps for audio guidance
      const steps = [];
      if (routeData.legs && routeData.legs[0] && routeData.legs[0].steps) {
        for (const step of routeData.legs[0].steps) {
          // OSRM returns distances in meters, coordinates in [lng, lat]
          let instruction = step.maneuver.modifier 
            ? `Tournez à ${step.maneuver.modifier.replace('left', 'gauche').replace('right', 'droite')}`
            : 'Continuez';
      ...
      console.warn('Failed to calculate fastest route with OSRM, falling back to straight line:', error);
      // Fallback to straight line
      const distance = getDistance(start, end);
      
      // Fallback speed based on profile
      let speed = 1.4; // default foot ~ 5kmh
      const profile = this.getOsrmProfile();
      if (profile === 'bike') speed = 5.5; // ~20kmh
      if (profile === 'driving') speed = 13.8; // ~50kmh
  ...
  private calculateSafestRoute(start: Coordinates, end: Coordinates, grid: RiskFieldGrid): Route | undefined {
    ...
    while (openSet.length > 0 && iterations < MAX_ITERATIONS) {
      ...
      if (current.id === endNodeId) {
        ...
        while (currId && !visitedIds.has(currId)) {
      ...
      for (const edge of graphNode.edges) {
        ...
        for (const incident of activeIncidents) {
          const distToIncident = getDistance(neighborGraphNode.coord, incident.location);
          if (distToIncident < 50) { // Penalize if within 50 meters
            if (incident.severity === IncidentSeverity.HIGH || incident.type === IncidentType.ROAD_CLOSED || incident.type === IncidentType.ACCIDENT) {
              // Hard block or huge penalty
              incidentPenalty += 10000; // Massive penalty to avoid
            } else if (incident.severity === IncidentSeverity.MEDIUM || incident.type === IncidentType.ROADWORKS || incident.type === IncidentType.DANGER) {
              // Soft avoid
              incidentPenalty += (50 - distToIncident) * 20;
            } else {
              incidentPenalty += (50 - distToIncident) * 5;
            }
          }
        }

        // Cost Function: distance + (risk_weight * risk_level) + incident_penalty
        // We square the risk to heavily penalize high risk areas
        const riskPenalty = Math.pow(riskValue, 2) * this.riskWeight * 10;
        const movementCost = edge.distance + riskPenalty + incidentPenalty;
