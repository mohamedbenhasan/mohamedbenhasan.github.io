import { Coordinates, Route, RiskFieldGrid, GridCell, IncidentSeverity } from '../types';
      ...
      for (const edge of graphNode.edges) {
        ...
        for (const incident of activeIncidents) {
          ...
          if (distToIncident < 50) { // Penalize if within 50 meters
            if (incident.severity === IncidentSeverity.HIGH || incident.type === IncidentType.ROADBLOCK || incident.type === IncidentType.ACCIDENT) {
              // Hard block or huge penalty
              incidentPenalty += 10000; // Massive penalty to avoid
            } else if (incident.severity === IncidentSeverity.MEDIUM || incident.type === IncidentType.CONSTRUCTION || incident.type === IncidentType.DANGER) {
              // Soft avoid
