  ...
  private static startListening() {
    ...
    this.unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach(doc => {
        ...
        // Filter out fictional/simulated messages (since we've passed the simulation phase)
        const isSimulated = data.involvedUserIds && data.involvedUserIds.some(id => id.startsWith('vru-') || id === 'user-agent');
        
        if (data.expiresAt > now && !isSimulated) {
